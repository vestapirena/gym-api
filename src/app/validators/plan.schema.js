...
const nameMx1to50 = Joi.string().trim().pattern(nameRegex).required().messages({
  ...
  'string.pattern.base': 'Nombre: letras (acentos), números y espacios (1 a 50)',
});

const descMx1to100 = Joi.string().trim().pattern(descRegex).required().messages({
  'any.required': 'La descripción es obligatoria',
  'string.empty': 'La descripción es obligatoria',
  'string.pattern.base': 'Descripción: letras (acentos), números, espacios, coma, punto y guión (1 a 100)',
});

const positiveDec = (msg) => Joi.number().greater(0).required().messages({
  'number.base': msg, 'number.greater': msg, 'any.required': msg,
});
const nonNegativeDec = (msg) => Joi.number().min(0).required().messages({
  'number.base': msg, 'number.min': msg, 'any.required': msg,
});
const positiveInt = (msg) => Joi.number().integer().greater(0).required().messages({
  'number.base': msg, 'number.greater': msg, 'number.integer': msg, 'any.required': msg,
});
